...
class SeparableConvBlock(nn.Module):
    ...
    def __init__(self, in_channels, out_channels=None, use_group_norm=False, group_norm_size=16):
        ...
        self.conv = DepthwiseSeparableConv(in_channels, out_channels, kernel_size=3, stride=1, padding=1,
                                           norm_layer=norm_layer, num_groups=num_groups, norm_kwargs=norm_kwargs,
                                           act_layer=None)
...
class BiFPNLayer(nn.Module):
    ...
    def __init__(
        ...
        self.conv4_down = SeparableConvBlock(num_channels, use_group_norm=use_group_norm,
                                             group_norm_size=group_norm_size)
        self.conv5_down = SeparableConvBlock(num_channels, use_group_norm=use_group_norm,
                                             group_norm_size=group_norm_size)
        self.conv6_down = SeparableConvBlock(num_channels, use_group_norm=use_group_norm,
                                             group_norm_size=group_norm_size)
        self.conv7_down = SeparableConvBlock(num_channels, use_group_norm=use_group_norm,
                                             group_norm_size=group_norm_size)

        if use_p2:
            self.conv3_down = SeparableConvBlock(num_channels, use_group_norm=use_group_norm,
                                                 group_norm_size=group_norm_size)
            self.conv2_up = SeparableConvBlock(num_channels, use_group_norm=use_group_norm,
                                               group_norm_size=group_norm_size)
        ...
        if self.first_time:
            self.p5_down_channel = Conv1x1Block(features[4], num_channels, use_group_norm=use_group_norm,
                                                group_norm_size=group_norm_size)
            self.p4_down_channel = Conv1x1Block(features[3], num_channels, use_group_norm=use_group_norm,
                                                group_norm_size=group_norm_size)
            self.p3_down_channel = Conv1x1Block(features[2], num_channels, use_group_norm=use_group_norm,
                                                group_norm_size=group_norm_size)
            ...
            self.p4_down_channel_2 = Conv1x1Block(features[3], num_channels, use_group_norm=use_group_norm,
                                                  group_norm_size=group_norm_size)
            self.p5_down_channel_2 = Conv1x1Block(features[4], num_channels, use_group_norm=use_group_norm,
                                                  group_norm_size=group_norm_size)

            if use_p2:
                self.p2_down_channel = Conv1x1Block(features[1], num_channels, use_group_norm=use_group_norm,
                                                    group_norm_size=group_norm_size)
                self.p3_down_channel_2 = Conv1x1Block(features[2], num_channels, use_group_norm=use_group_norm,
                                                      group_norm_size=group_norm_size)
