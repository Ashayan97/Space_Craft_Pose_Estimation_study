...
_C.DATASET.ROOT      = 'Datasets'      # Root directory of all datasets
...
_C.TRAIN.VALID_FRACTION = 0.2                              # Fraction of validation set on which inference is conducted

# - Batches
_C.TRAIN.IMAGES_PER_GPU = 16                                # Batch size PER GPU, NOT across all GPUs!
_C.TRAIN.SHUFFLE        = True
_C.TRAIN.WORKERS        = 4

# - CSV files
_C.TRAIN.TRAIN_CSV  = 'synthetic\\train.json'
_C.TRAIN.VAL_CSV    = 'synthetic\\validation.json'
...
_C.TEST.TEST_CSV = 'lightbox/lightbox.csv'           # CSV file to containing images to test on
...
# ------------------------------------------------------------------------------ #
_C.ODR = CN()
_C.ODR.MIN_ENTROPY       = True                            # Perform ODR?
...
# ------------------------------------------------------------------------------ #
_C.MODEL = CN()
_C.MODEL.PRETRAIN_FILE     = 'spnv2_efficientnetb3_fullconfig_offline.pth'                           # Path to pre-trained model to load
_C.MODEL.EFFICIENTDET_PHI  = 3                              # EfficientNet scaling parameter
...
_C.MODEL.BACKBONE.NAME = 'efficientdet_d0'                  # Name of the backbone: 'efficientdet_d#'
_C.MODEL.BACKBONE.PRETRAINED = 'efficientdet_d0.pth'                         # Pre-trained model for the backbone only
...
_C.MODEL.HEAD.EFFICIENTPOSE_LOSS_FACTOR = [1, 1, 1]              # Loss scaling factors for EfficientPose head: [cls, bbox, pose]
_C.MODEL.HEAD.ANCHOR_SCALE = '[2 ** 0, 2 ** (1.0 / 3.0), 2 ** (2.0 / 3.0)]'
_C.MODEL.HEAD.ANCHOR_RATIO = [[1.0, 1.0], [1.4, 0.7], [0.7, 1.4]]
...
def update_config(cfg, args):
    ...
    cfg.DATASET.CAMERA    = join(cfg.DATASET.ROOT, cfg.DATASET.CAMERA)
